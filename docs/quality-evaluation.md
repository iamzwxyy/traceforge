...
| Gate-aware intent and human control | conversation ends as an answer without false evidence; the automatic gate controls plan review while Manual/Automatic/workspace Full access independently control actions; hard denials never upgrade | direct-answer, cross-gate permission matrix, stale-approval, sandbox fallback, and plan-gate state-machine tests |
...
| Durable, redacted, and boundary-safe output | only accepted public result fields stream; semantic provider data bearing credentials never persists or executes; JSON structure cannot synthesize a key; SQLite/REST/WebSocket and native snapshots fail closed; retry/cancel/restart/truncation cannot merge attempts; terminal DB faults cannot manufacture a canonical run/turn pair | adjacent/every-prefix/escaped/structural secret tests, provider ingress and tool-result metadata cases, legacy approval invalidation, REST/WebSocket byte assertions, direct/verified completion, retry isolation, restart recovery, cancellation, and atomic terminal fault injection |
