...
try {
  ...
  await page.getByRole("heading", { name: /decisions change/ }).waitFor();
  ...
  await page.getByRole("button", { name: "Approve & build" }).waitFor();
  ...
  await page.getByRole("heading", { name: "Work proven, not merely reported" })
  ...
  await caption(page, "TraceForge — Work proven, not merely reported");
  await page.waitForTimeout(3500);
} finally {
  await context?.close();
  await browser?.close();
